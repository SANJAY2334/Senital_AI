...
export const ActivityTimelineChart: React.FC<ActivityTimelineChartProps> = ({
  ...
  if (!data || data.length === 0) {
    return (
      <div className="flex items-center justify-center h-44 text-xs font-mono text-slate-500">
  ...
  return (
    <div className="relative w-full overflow-hidden select-none">
        ...
        {points.map((p, i) => {
          ...
          return (
            <g key={i}>
              ...
              {p.alertCount > 0 && <circle cx={p.x} cy={p.y - 6} r="2.5" fill="#EF4444" />}
      ...
      {hoverIndex !== null && points[hoverIndex] && (
        <div
          className="absolute pointer-events-none bg-[#090E1A] border border-[#1E293B] shadow-xl px-2.5 py-1.5 rounded-md text-[11px] font-mono z-20 space-y-0.5"
          style={{
        ...
        >
          <div className="text-slate-400 font-semibold">{points[hoverIndex].timeLabel} UTC</div>
          <div className="text-cyan-400 font-medium">
            Events: {points[hoverIndex].eventCount.toLocaleString()}
          </div>
          {points[hoverIndex].alertCount > 0 && (
            <div className="text-red-400 font-medium">Alerts: {points[hoverIndex].alertCount}</div>
          )}
        </div>
      )}
    </div>
