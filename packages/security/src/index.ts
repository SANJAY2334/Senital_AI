...
export class SentinelSecurityService implements ISecurityService {
  ...
  constructor(masterKeyHex?: string) {
    const keyHex = masterKeyHex || process.env.KMS_MASTER_KEY_ID || '0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef';
    this.masterKey = Buffer.from(keyHex.substring(0, 64), 'hex');
  }

  async encryptTenantPayload(tenantId: string, payload: Buffer | string): Promise<EncryptedEnvelope> {
    ...
    const tenantKey = crypto.hkdfSync('sha256', this.masterKey, Buffer.from(tenantId), Buffer.from('sentinelai-tenant-key'), 32);
  ...
  async decryptTenantPayload(envelope: EncryptedEnvelope): Promise<Buffer> {
    const tenantKey = crypto.hkdfSync('sha256', this.masterKey, Buffer.from(envelope.tenantId), Buffer.from('sentinelai-tenant-key'), 32);
    const decipher = crypto.createDecipheriv('aes-256-gcm', Buffer.from(tenantKey), Buffer.from(envelope.iv, 'base64'));
  ...
  validateAssetAccess(tenantId: string, assetCriticality: string, requestedAction: string): boolean {
    ...
    if (assetCriticality === 'Critical Infrastructure' && requestedAction.startsWith('AUTOMATED_')) {
